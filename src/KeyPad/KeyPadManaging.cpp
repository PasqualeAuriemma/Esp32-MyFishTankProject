...
byte KeyPad::getKeyPadValue()
{
  ...
  {
    if (analog_voltage >= 0 && analog_voltage < 21)
    { // left
      return codeKeys[0];
    }
    else if (analog_voltage >= 21 && analog_voltage < 61)
    { // up
      return codeKeys[3];
    }
    else if (analog_voltage >= 61 && analog_voltage < 121)
    { // down
      return codeKeys[2];
    }
    else if (analog_voltage >= 121 && analog_voltage < 191)
    { // right
      return codeKeys[1];
    }
    else
    { // if(analog_voltage > 191 && analog_voltage < 280){ //ok
      return codeKeys[4];
    }
  }
  else
